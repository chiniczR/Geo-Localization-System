...
namespace PL
{
    public class KMeansClustering
    {
        ...
        private static double[][] Normalized(double[][] rawData)
        {
            ...
            for (int j = 0; j < result[0].Length-1; ++j) // each col
            {
                double colSum = 0.0;
        ...
        private static bool UpdateMeans(double[][] data, int[] clustering, double[][] means)
        {
            ...
            for (int k = 0; k < means.Length-1; ++k)
                for (int j = 0; j < means[k].Length-1; ++j)
                    means[k][j] = 0.0;
        ...
        private static bool UpdateClustering(double[][] data, int[] clustering, double[][] means)
        {
            ...
            for (int i = 0; i < data.Length-1; ++i) // walk thru each tuple
            {
                for (int k = 0; k < numClusters; ++k)
        ...
        public static List<KeyValuePair<int,double[]>> Clustered(double[][] data, int[] clustering, int numClusters, int decimals)
        {
            List<KeyValuePair<int, double[]>> res = new List<KeyValuePair<int, double[]>>();
            for (int k = 0; k < numClusters; ++k)
            {
                //Console.WriteLine("==================="); // Start of cluster
                for (int i = 0; i < data.Length-1; ++i)
                {
                    int clusterID = clustering[i];
                    if (clusterID != k) { continue; }
                    //Console.Write(i.ToString().PadLeft(3) + " "); // Index
                    double lat = data[i][0];
                    double lon = data[i][1];
                    if (lat >= 0.0 && lon >= 0.0)
                    {
                        res.Add(new KeyValuePair<int, double[]>(clusterID, new double[] {lat, lon}));
                    }
                    //for (int j = 0; j < data[i].Length; ++j)
                    //{
                        //if (data[i][j] >= 0.0) Console.Write(" ");
                        //Console.Write(data[i][j].ToString("F" + decimals) + " ");
                        // The actual clustered data
                    //}
                    //Console.WriteLine("");
                }
        ...
        public static List<KeyValuePair<int, float[]>> ClustersAndMeans(double[][] rawData, int numClusters = 3)
        {
            int[] clustering = Cluster(rawData, numClusters);
            List<KeyValuePair<int, double[]>> clustered = Clustered(rawData, clustering, numClusters, 1).OrderBy(kvPair => { return kvPair.Key; }).ToList();
            List<KeyValuePair<int, float[]>> clustersMeans = new List<KeyValuePair<int, float[]>>();
            int i = 0;
            int numOfClusters = clustered.GroupBy(kv => { return kv.Key; }).Select(group => group.Key).Count();
            while(i < numOfClusters + 1)
            {
                List<KeyValuePair<int,double[]>> currentCluster = clustered.FindAll(kvPair => { return kvPair.Key == i; });
                if(currentCluster.Count() > 0)
                {
                    KeyValuePair<int, float[]> toAdd = ClusterMean(currentCluster);
                    clustersMeans.Add(toAdd);
                }
                i++;
            }
        ...
        private static KeyValuePair<int,float[]> ClusterMean(List<KeyValuePair<int, double[]>> leCluster)
        {
            List<double> Xcoords = new List<double>();
            List<double> Ycoords = new List<double>();
            int clusterId = leCluster.FirstOrDefault().Key;
            foreach (var kv in leCluster)
            {
                Xcoords.Add(kv.Value[0]);
                Ycoords.Add(kv.Value[1]);
            }
            float Xmean = 0.0f;
            float.TryParse(Xcoords.Average().ToString(), out Xmean);
            float Ymean = 0.0f;
            float.TryParse(Ycoords.Average().ToString(), out Ymean);
            return new KeyValuePair<int, float[]>(clusterId, new float[] { Xmean, Ymean });
        }
    } // Program
}
